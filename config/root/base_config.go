...
import (
	_ "dubbo.apache.org/dubbo-go/v3/common/config"
	"dubbo.apache.org/dubbo-go/v3/common/constant"
	_ "dubbo.apache.org/dubbo-go/v3/common/yaml"
)

// Config is the common configuration for provider and consumer
type Config struct {
	// ConfigCenterConfig config center config
	ConfigCenterConfig *config.ConfigCenterConfig `yaml:"config_center" json:"config_center,omitempty"`

	// Remotes since 1.5.0 version
	Remotes map[string]*config.RemoteConfig `yaml:"remote" json:"remote,omitempty" property:"remote"`

	// ServiceDiscoveries service discovery config
	ServiceDiscoveries map[string]*discovery.Config `yaml:"serviceDiscovery" json:"serviceDiscovery,omitempty" property:"serviceDiscovery"`

	// MetadataReport metadata report config
	MetadataReport *report.Config `yaml:"metadata-report" json:"metadata-report,omitempty" property:"metadata-report"`

	// application config
	Application *application.Config `yaml:"application" json:"application,omitempty" property:"application"`
    // metric config
	MetricConfig        *metric.Config `yaml:"metric" json:"metric,omitempty"`
	// prefix              string
	...
	CacheFile bool `yaml:"cache-file" json:"cache-file,omitempty" property:"cache-file"`
}

func (c *Config) Prefix() string {
	return constant.ConfigRootPrefix
}

// GetServiceDiscoveries service discovery
func (c *Config) GetServiceDiscoveries(name string) (config *discovery.Config, ok bool) {
	config, ok = c.ServiceDiscoveries[name]
	return
}

// GetRemoteConfig will return the remote's config with the name if found
func (c *Config) GetRemoteConfig(name string) (config *config.RemoteConfig, ok bool) {
...
//}
//
//func (c *Config) freshInternalConfig(config *config.InmemoryConfiguration) {
...
func (c *Config) SetFatherConfig(fatherConfig interface{}) {
